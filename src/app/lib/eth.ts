...
import { contractAddress, chainId } from '../lib/constants'; // ✅ 수정된 부분
...
export async function connectWallet(): Promise<string> {
  ...
  if (!injected || !('request' in injected)) {
    throw new Error('지갑(예: MetaMask)을 찾을 수 없습니다.');
  }
  const accounts = (await injected.request({
    method: 'eth_requestAccounts',
  })) as string[];
  if (!accounts?.[0]) throw new Error('지갑 계정을 가져올 수 없습니다.');
  return accounts[0];
}
...
export async function readContractBalance(): Promise<string> {
  ...
  const raw = (await (contract as any).getBalance()) as bigint;
  return formatEther(raw);
}

export async function readOwner(): Promise<string> {
  const contract = await getContract(false);
  return (await (contract as any).owner()) as string;
}

export async function sendTip(amountEth: string): Promise<string> {
  await ensureNetwork();
  const contract = await getContract(true);
  const tx = await (contract as any).tip({ value: parseEther(amountEth) });
...
export async function withdrawTips(): Promise<string> {
  await ensureNetwork();
  const contract = await getContract(true);
  const tx = await (contract as any).withdrawTips();
  const receipt = await tx.wait();
  return receipt?.hash ?? tx.hash;
}
